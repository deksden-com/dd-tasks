...
Для обычного протокола следующий логический stage - `specify`: уточнение задачи в problem space. Для интерактивного протокола следующий stage - `interactive`: пользователь и агент сразу работают в выбранном Git-контуре, а rationale изменений фиксируется в протоколе.
