...
| `PRT-004 source package`: one built Hono process serves API and Vite SPA on one external port with internal PostgreSQL; guarded lifecycle, readiness and immutable revision/digest work under exact preview binding | `SCN-003-private-preview-runtime` | accepted feature HEAD recorded by the source passport | `local-container` | `applicable` | `pending_readiness` | `pending` | `pending_source_passport` | `CODE/readiness` | `pnpm scenario:preview` for both `preview-checkpoint` and `preview-eval-output`; fresh API/browser role matrix; wrong-binding negative; readiness-before-init; restart or exact volume cleanup; full quality/docs/DB checks | `.memory-bank/protocol/PRT-004-exe-preview-runtime/evidence/verification-passport.md` | Does not prove Exe.dev, provider identity/team/VM/share/transport/capacity, CI/CD, production availability, backup or public sharing. |
